...
with st.sidebar:
  ...
  user_input = st.text_input(
      "輸入台股代碼 (例: 2330, 2303, 2603)",
      value="2330",
      placeholder="輸入 4 碼代號",
  )

  # 處理代碼格式
  clean_code = "".join(filter(str.isdigit, user_input))
  if len(clean_code) == 4:
    stock_symbol = clean_code + ".TW"
    company_name = TW_STOCK_NAMES.get(clean_code, f"台股代號 {clean_code}")
  elif len(clean_code) == 5:
    stock_symbol = clean_code + ".TWO"
    company_name = f"櫃買代號 {clean_code}"
  else:
  ...
  for idx_name, sym in {
    ...
    try:
      ...
      st.text(f"{idx_name}: 數據連線中...")

# 主程式邏輯
if stock_symbol:
  try:
    with st.spinner(f"正在載入 {company_name} ({stock_symbol}) 歷史資料..."):
      stock_data = yf.download(stock_symbol, period=period, progress=False)

    if stock_data.empty:
      st.error(
          f"❌ 找不到代碼 `{stock_symbol}` 的資料，請確認輸入是否正確（上市請輸入 4 碼）。"
      )
    else:
      ...
      tab1, tab2, tab3, tab4 = st.tabs(
          ["📊 三竹風 K線與買賣訊號", "📈 加粗均線與技術指標", "🎯 當沖/隔日沖與目標價", "📰 相關財經新聞"]
      )
      ...
      with tab3:
        ...
        with col_s1:
          ...
          if tail_p > 0.65 and vol_s > 1.2:
            st.success(
                "🔥 **強烈推薦隔日沖**：今日尾盤帶量上拉，具備隔日開高優勢。"
            )
          else:
            st.info("⚪ **一般盤勢**：今日量價結構平穩，適合依技術支撐操作。")
          st.write(f"- 尾盤收拉強度: `{tail_p:.2f}` (大於 0.65 佳)")
          st.write(f"- 成交量放大倍數: `{vol_s:.2f}x`")

        with col_s2:
          st.markdown("#### 🎯 目標價與停損點設定")
          st.metric("建議進場參考價", f"${current_price:.2f}")
          st.metric("短期波段目標價 (+1.5倍波動)", f"${short_target:.2f}")
          st.metric("長期核心目標價 (+12%)", f"${long_target:.2f}")
          st.metric("建議嚴格停損價", f"${stop_loss:.2f}")

      # 標籤 4: 新聞與網站
      with tab4:
        st.subheader("📰 即時財經新聞與專業看盤網站")
        try:
          n_items = ticker.news
          if n_items:
            for n in n_items[:6]:
              st.markdown(
                  f"- **[{n.get('title','新聞')}]({n.get('link','#')})** — *{n.get('publisher','Yahoo')}*"
              )
          else:
            st.info("目前無即時推播新聞。")
        except:
          st.info("無法直接取得該標的新聞清單。")

        st.markdown("---")
        st.markdown("#### 🔗 推薦台灣與國際專業財經平台")
        st.markdown(
