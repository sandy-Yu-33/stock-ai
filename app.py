...
with st.sidebar:
  ...
  stock_symbol = st.text_input(
      "輸入股票代碼 (例如: 2330, 2344.TW, AAPL)",
      value="2330",
      placeholder="輸入股票代碼",
  )

  # 自動判斷：如果輸入的是4位數字（台股代碼），自動加上 .TW
  if stock_symbol.isdigit() and len(stock_symbol) == 4:
    stock_symbol = stock_symbol + ".TW"

  # 時間範圍選擇
  time_range = st.selectbox(
      "選擇時間範圍", ["1個月", "3個月", "6個月", "1年", "2年", "5年"]
  )
...
if stock_symbol:
  try:
    ...
    if stock_data.empty:
      ...
      with col1:
        current_price = info.get(
            "currentPrice", stock_data["Close"].iloc[-1]
        )
        if hasattr(current_price, "iloc"):
          current_price = current_price.iloc[0]
        st.metric("當前價格", f"${float(current_price):.2f}")
      ...
      with col2:
        ...
        prev_val = (
            prev_close.iloc[0] if hasattr(prev_close, "iloc") else prev_close
        )
        change = float(current_val) - float(prev_val)
        change_percent = (
            (change / float(prev_val) * 100) if float(prev_val) else 0
        )
      ...
      with tab2:
        ...
        if plotly_available:
          ...
          st.line_chart(stock_data[["Close", "MA20", "MA50"]])
      ...
      with tab3:
        ...
        with col3:
          predicted_change = (
              predictions[-1] - stock_data["Close"].iloc[-1]
    ...
    st.info("請確保輸入的股票代碼正確，例如: 2330 (台積電), AAPL (美國)")
